    ...
    with metrics.record_counter('organization_members') as counter:
        ...
        for response in authed_get_all_pages(
            ...
            for member in organization_members["values"]:
                ...
                with singer.Transformer() as transformer:
                    team_members_rec = transformer.transform(member, schemas, metadata=metadata.to_map(mdata))
...
def get_all_pull_requests(schemas, repo_path, state, mdata, start_date):
    ...
    with metrics.record_counter('pull_requests') as counter, metrics.record_counter('pull_request_details') as details_counter:
        with metrics.record_counter('pull_request_comments') as pull_request_comments_counter:
            for response in authed_get_all_pages(
                    'pull_requests',
                ...
                for pr in pull_requests:
                    ...
                    # once we find the first piece of old data we can return, thanks to
                    # the sorting
                    if bookmark_time and singer.utils.strptime_to_utc(pr.get('updated_on')) < bookmark_time:
                        return state
                    ...
                    if schemas.get('pull_request_comments'):
                        for review_rec in get_comments_for_pr(pr_id, schemas['pull_request_comments'], repo_path, state, mdata['pull_request_comments']):
                            singer.write_record('pull_request_comments', review_rec, time_extracted=extraction_time)
                            singer.write_bookmark(state, repo_path, 'pull_request_comments', {'since': singer.utils.strftime(extraction_time)})
                    ...
                    if schemas.get('pull_request_stats'):
                        for pull_request_stats in get_pull_request_stats(pr_id, schemas['pull_request_stats'], repo_path, state, mdata['pull_request_stats']):
                            singer.write_record('pull_request_stats', pull_request_stats, time_extracted=extraction_time)
                            singer.write_bookmark(state, repo_path, 'pull_request_stats', {'since': singer.utils.strftime(extraction_time)})
...
def get_comments_for_pr(pr_number, schema, repo_path, state, mdata):
    for response in authed_get_all_pages(
...
def get_pull_request_stats(pr_number, schema, repo_path, state, mdata):
    for response in authed_get_all_pages(
            'pull_request_stats',
        ...
        for stat in stats["values"]:
            stat['pr_id'] = pr_number
            stat['_sdc_repository'] = repo_path
            with singer.Transformer() as transformer:
                rec = transformer.transform(stat, schema, metadata=metadata.to_map(mdata))
            yield rec
        return state
...
def get_pull_request_files(pr_id, schema, repo_path, state, mdata):
...
def get_all_commits(schema, repo_path,  state, mdata, start_date):
    ...
    with metrics.record_counter('commits') as counter:
        for response in authed_get_all_pages(
                'commits',
                f'{BASE_URL}/repositories/{repo_path}/commits{query_string}'
        ):
...
def get_all_deployments(schema, repo_path, state, mdata, _start_date):
    ...
    with metrics.record_counter('deployments') as counter:
        for response in authed_get_all_pages(
                'deployments',
                f'{BASE_URL}/repositories/{repo_path}/deployments'
        ):
...
SUB_STREAMS = {
    'pull_requests': ['pull_request_comments', 'review_comments', 'pull_request_stats', 'pull_request_details', 'pull_request_files'],
            ...
            if stream_id in selected_stream_ids:
                ...
                if not sub_stream_ids:
                    if stream_id == "organization_members":
                        state = sync_func(stream_schemas, workspace, state, mdata, start_date)
                    else:
                ...
                # handle streams with sub streams
                    ...
                    if stream_id == "organization_members":
                        state = sync_func(stream_schemas, workspace, state, stream_mdata, start_date)
                    else:
